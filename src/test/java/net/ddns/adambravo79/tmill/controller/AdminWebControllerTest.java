        ...
        void falaT1000Tts_withValidText_shouldReturnSuccess() throws Exception {
                ...
                try (var mockedFiles = mockStatic(Files.class)) {
                        mockedFiles.when(() -> Files.write(any(Path.class), any(byte[].class)))
                                        .thenReturn(tempPath);
                        mockedFiles.when(() -> Files.move(any(Path.class), any(Path.class)))
                        ...
                        mockMvc.perform(
                                        ...
                                        .andExpect(content().string(containsString(
                                                        "Áudio enviado com sucesso para o chat " + chatId)));

                        // Verifica
                        verify(azureTtsClient).synthesizeFullText(text);
                        verify(telegramFacade).enviarMidia(
                                        eq(chatId),
                                        anyString(),
                                        eq("🔊 Áudios para a futura Skynet"));
                }
        }

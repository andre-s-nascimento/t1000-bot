...
public class TmdbClient {
  ...
  public MovieSearchResponse pesquisarFilme(String query) {
    ...
    try {
      ...
      log.error("❌ TMDB: erro na busca query='{}'", LogSanitizer.sanitizeQuery(queryFinal), e);
      throw e;
    }
  }
  ...
  public MovieRecord buscarDetalhes(Long movieId) {
    ...
    MovieRecord response =
        restClient
            ...
            .uri(
                uriBuilder ->
                    uriBuilder.path("/movie/{id}").queryParam(LANGUAGE, PT_BR).build(movieId))
            .retrieve()
...
<<<<<<< HEAD
  ...
  public TvRecord buscarDetalhesSerie(Long tvId) {
    ...
    TvRecord response =
        restClient
            .get()
            .uri(uriBuilder -> uriBuilder.path("/tv/{id}").queryParam(LANGUAGE, PT_BR).build(tvId))
            .retrieve()
            .body(TvRecord.class);
    if (response == null) {

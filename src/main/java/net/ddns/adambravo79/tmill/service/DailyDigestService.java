...
>>>>>>> 6286d61 (feat: ajustes de refatoracao, ajustes nos testes)
  ...
  private void generateDigest(
    ...
    try {
      ...
      log.error("❌ Erro HTTP do Groq ao gerar digest {}: {}", periodLabel, e.getStatusCode(), e);
      // Rate limit ou erro de autenticação — não retry aqui, apenas log

    } catch (GroqRateLimitException e) {
      log.error(
          "❌ Rate limit do Groq ao gerar digest {}. Considerar retry agendado.", periodLabel, e);
      ...
      throw new DigestGenerationException("Erro inesperado ao gerar digest: " + periodLabel, e);
    }
  }
...
>>>>>>> 6286d61 (feat: ajustes de refatoracao, ajustes nos testes)
    ...
    for (ChatMessage msg : messages) {
      ...
      if (previous != null) {
        ZoneId zone = ZoneId.of(BRAZIL_ZONE);
        long diff = Duration.between(previous.atZone(zone), current.atZone(zone)).toMinutes();
        if (diff >= 20) {
          sb.append("\n==============================\n");
          sb.append("NOVO BLOCO DE CONVERSA\n");
          sb.append("==============================\n\n");
  ...
  private String truncateIfNeeded(String finalMessages) {
    ...
    log.warn(
        "✂️ Mensagens truncadas de {} para {} chars", finalMessages.length(), allowedMessagesSize);
...
>>>>>>> 6286d61 (feat: ajustes de refatoracao, ajustes nos testes)
  ...
  private void sendDigestToChat(Long chatId, String finalMessage) {
    try {
      ...
      throw new DigestSendException("Erro inesperado no envio do digest para chatId=" + chatId, e);
    }
  }
  ...
  private void sendChunk(Long chatId, String chunk) {
    try {
      ...
      if (isHtmlParseError(e)) {
        log.warn(
            "⚠️ Erro de parse HTML (BadRequest), reenviando como texto puro para" + " chatId={}",
            chatId);
        try {
      ...
      throw new DigestSendException("Falha de conectividade com Telegram para chatId=" + chatId, e);
    }
  }
